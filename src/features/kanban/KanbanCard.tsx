  ...
  return (
    <div
      ...
      style={{
        ...
        padding: '12px', cursor: 'grab', opacity: isDragging ? 0 : 1,
      }}
    >
      <p style={{ margin: '0 0 8px', fontSize: '13px', fontWeight: 700, color: '#0f172a' }}>{task.title}</p>
      <span style={{ background: pill.bg, color: pill.text, fontSize: '10px', fontWeight: 800, padding: '2px 8px', borderRadius: '999px' }}>{pill.label}</span>
    </div>
  );
}